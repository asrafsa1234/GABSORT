    ...
    return (
        ...
        >
            <div className={`p-2 rounded-full transition-colors duration-200 ${isActive ? 'bg-green-500' : 'bg-transparent'}`}>
                <Icon className={`w-6 h-6 transition-colors duration-200 ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-gray-600'}`} />
            </div>
            <span className={`text-xs font-medium transition-colors duration-200 ${isActive ? 'text-green-600' : 'text-gray-500 group-hover:text-gray-700'}`}>{label}</span>
        </button>
    );
}
...
const BottomNav: React.FC<BottomNavProps> = ({ activeView, setView }) => {
    return (
        <nav className="fixed bottom-0 inset-x-0 bg-white rounded-t-2xl shadow-[0_-2px_15px_rgba(0,0,0,0.08)] z-50">
                ...
                <div className="w-1/4 h-full"><NavButton view='profile' label='Profile' Icon={ProfileIcon} activeView={activeView} setView={setView} /></div>
                 ...
                 <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 flex items-center justify-center pointer-events-none">
                     <button
                        ...
                        className="pointer-events-auto -translate-y-8 flex items-center justify-center w-16 h-16 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full shadow-lg text-white transform hover:scale-110 transition-transform ring-4 ring-white"
